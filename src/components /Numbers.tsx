...
function Hours({
	...
	const { numbersOuter, numbersInner } = useMemo(() => {
		const { numbers: outer, numbersInner: inner } = CLOCK_VALUES[mode]
		...
		return {
			numbersOuter: outer.map((value, i) => ({
				value,
				enabled:
					disabledTimeRangeValidator?.validateHour(normalizeOuterIndex(i)) ?? true,
			})),
			numbersInner: inner?.map((value, i) => ({
				value,
				enabled:
					disabledTimeRangeValidator?.validateHour(normalizeInnerIndex(i)) ?? true,
			})),
		}
...
function Minutes({ anim, hour, disabledTimeRangeValidator }: MinuteProps) {
	...
	const minutes = useMemo(() => {
		return MINUTES.map(value => ({
			value,
			enabled:
				disabledTimeRangeValidator?.validateMinute(hour, parseInt(value, 10)) ?? true,
		}))
